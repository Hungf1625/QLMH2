...
// Hủy yêu cầu
$query = "DELETE FROM groupmember WHERE group_id = ? AND user_id = ? AND status = 'pending'";
$stmt = $pdo->prepare($query);
$result = $stmt->execute([$group_id, $target_user_id]);

if ($result && $stmt->rowCount() > 0) {
    echo "<script>
        alert('Đã hủy yêu cầu tham gia nhóm thành công!');
        window.location.href = '../views/groupDetail.php?group_id=$group_id';
    </script>";
} else {
    echo "<script>
        alert('Không thể hủy yêu cầu hoặc yêu cầu không tồn tại');
        window.location.href = '../views/groupDetail.php?group_id=$group_id';
    </script>";
}
exit;
